...
g <- ggplot(roll(m = m_, n = n_),
  ...
  labs(
    ...
    x = 'valores posibles de cada tirada',
    y = 'número de veces que ha salido el valor en porcentaje'
  ) +
...
g2 <- ggplot(roll(m = m_2, n = n_2),
            mapping = aes(x = V1)) +
  labs(
    subtitle = str_interp('Densidad de las medias del lanzamiento de ${n_2} dados ${m_2} veces'),
    x = 'valores posibles de cada tirada',
    y = 'número de veces que ha salido el valor en porcentaje'
  ) +
  geom_bar(aes(y = ..prop..), alpha = 0.4) +
...
g3 <- ggplot(roll(m = m_3, n = n_3),
            mapping = aes(x = V1)) +
  labs(
    subtitle = str_interp('Densidad de las medias del lanzamiento de ${n_3} dados ${m_3} veces'),
    x = 'valores posibles de cada tirada',
    y = 'número de veces que ha salido el valor en porcentaje'
  ) +
  geom_bar(aes(y = ..prop..), alpha = 0.4) +
...
```{r}
print("Ambos escenarios son probables, pues con un número de lanzamientos tan elevado, ambos tenderán a buscar un valor promedio cercano de 10. Si tenemos que argumentar ambos escenarios, en el primero, podremos visualizar mayor variabilidad, pues cada tirada depende de un único dado, mientras que en el escenario dos tenderá a apuntalarse los resultados, siendo más estables y concentrada alrededor del valor esperado. Dicho esto, el escenario sería más probable ya que la suma de las puntuaciones es más estable y superior al valor requerido de promedio superior a 5.")
...
loaded_dice <- function(n = 1) {
  faces <- 1:20
  probabilities <- c(rep(2/25, 5), rep(1/25, 15))
  result <- sample(faces, size = n, replace = TRUE, prob = probabilities)
  
  # Devolvuelve un dataframe
  return(data.frame(V1 = result))
}



loaded_dice5 <- function(n = 5) {
  faces <- 1:20
  probabilities <- c(rep(2/25, 5), rep(1/25, 15))
  result <- sample(faces, size = n, replace = TRUE, prob = probabilities)
...
* **Ejercicio 7**: Realiza un experimento simulando 5000 tiradas de 1 único dado trucado de 20 caras. Los resultados deben mostrarse en
un gráfico de barras igual que el de la Pregunta 2.
...
# Convertierte a dataframe
dados_trucados <- data.frame(V1 = loaded_dice(m_t))

# gráfico
d_trucado <- ggplot(dados_trucados,
            mapping = aes(x = V1)) +
  labs(
    subtitle = str_interp('Densidad de las medias del lanzamiento de ${n_t} dados ${m_t} veces'),
    x = 'valores posibles de cada tirada',
    y = 'número de veces que ha salido el valor en porcentaje'
  ) +
  geom_bar(aes(y = ..prop..), alpha = 0.4) +
...
* **Ejericio 8**: Realiza un experimento simulando 5000 tiradas de 5 dados trucados de 20 caras. Los resultados deben mostrarse en un 
gráfico de barras igual que el de la Pregunta 2.
...
# Obtener resultados del dado trucado y convertirlo a dataframe
dados_trucados <- data.frame(V1 = loaded_dice5(m_t5))

# Crear gráfico
d_trucado5 <- ggplot(dados_trucados,
            mapping = aes(x = V1)) +
  labs(
    subtitle = str_interp('Densidad de las medias del lanzamiento de ${n_t5} dados ${m_t5} veces'),
    x = 'valores posibles de cada tirada',
    y = 'número de veces que ha salido el valor en porcentaje'
  ) +
  geom_bar(aes(y = ..prop..), alpha = 0.4) +
...
```{r}
print("En este caso es poco probable que ningun escenario gane al final boss, pues la esperanza será inferior a  10, dado que el dado trucado mantiene probabilidades más elevadas en los números 1 al 5. Comentada la baja probabilidad de ambos, el escenario 2 sería más probable, ya que hay menor variabilidad, bajo la premisa que estará centrado en torno un promedio por debajo del umbral, teniendo una probilidad muy baja de ganar.")
